...
const BeverageMenuComponent = () => {
    const alkoDrinks = [
        ...
        'Prosecco Frizzante',
        'Aperol Spritz',
        'Gin Tonic (Gordon\'s klasický / růžový)',
        'Vodka Amundsen Expedition 40 %',
        'Rum Captain Morgan Spiced',
        'Jägermeister',
        'Rum Legendario Elixír 34 %',
    ...
    const nealkoDrinks = [
        ...
        'Limonáda (sirup, perlivá voda)',
        'Cola',
        'Sprite',
        'Tonic River / Fever Tree',
        'Red Bull'
    ];
                ...
                <section className="mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl mx-auto">
                        {alkoDrinks.map((drink, index) => (
                            <div key={drink} className={` backdrop-blur-sm rounded-lg p-4 ${index % 2 === 0 ? 'border-blush-200/30' : 'border-sage-200/30'}`}>
                ...
                <section className="mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl mx-auto">
                        {nealkoDrinks.map((drink, index) => (
                            <div key={drink} className={` backdrop-blur-sm rounded-lg p-4 ${index % 2 === 0 ? 'border-sage-200/30' : 'border-blush-200/30'}`}>
                                <h3 className="font-sans font-medium text-foreground text-center text-gray-900">{drink}</h3>
                            </div>
                        ))}
                    </div>
